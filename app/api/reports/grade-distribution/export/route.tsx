...
export async function GET(req: NextRequest) {
  try {
    ...
    const filename = buildExportFilename("grade-distribution-report", format);
    ...
    if (format === "pdf") {
      ...
      const pdfBuffer = await renderToBuffer(
        <ReportTemplate
          ...
          subtitle="Grade breakdown across all published results"
          headers={CSV_HEADERS}
          rows={rows}
          logo={logo}
        />
    ...
    return prismaErrorResponse(error, "Failed to export grade distribution report");
  }
}
