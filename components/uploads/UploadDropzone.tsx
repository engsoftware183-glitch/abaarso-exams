...
export function UploadDropzone({ id, title, description, accept, disabled, children, onSelect }: UploadDropzoneProps) {
  ...
  return (
    <label
      htmlFor={id}
      className={`flex cursor-pointer flex-col items-center justify-center rounded-lg border border-dashed border-[#CBD5E1] bg-white p-6 text-center transition hover:border-[#16A34A] hover:bg-[#F8FAFC] ${disabled ? "cursor-not-allowed opacity-60" : ""}`}
    ...
    >
      <UploadCloud className="h-8 w-8 text-[#16A34A]" aria-hidden="true" />
