...
// ======================================================
// TYPES (only fields returned by GET /api/reports/courses)
// ======================================================
...
export function CoursesReport() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await apiClient.get<CoursesReportResponse>(`/api/reports/courses?${params.toString()}`);
      setRows(data.data ?? []);
  ...
  async function handleExport(format: ExportFormat) {
    ...
    const url = `/api/reports/courses/export?${params.toString()}`;
    window.open(url, "_blank");
  }

  if (!isManager) {
    return (
      <AppShell title="Course Report" description="List of courses with department and semester details.">
      ...
      <div className="grid gap-6">
        ...
        <section className="grid gap-4 sm:grid-cols-3">
          {[
            ...
            { label: "Courses on this page", value: rows.length },
        ...
        <section className="overflow-hidden rounded-lg border border-[#E5E7EB] bg-white shadow-sm">
          {error ? (
            <div className="p-4">
              <ErrorState title="Failed to load the course report" message={error} onRetry={() => void load()} />
            </div>
          ) : loading ? (
            ...
            <div className="p-6">
              <EmptyState
                ...
                message={hasFilters || search ? "Try adjusting your search or filters." : "No courses have been added yet."}
              />
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full min-w-[800px] text-left text-sm">
                ...
                <tbody className="divide-y divide-[#E5E7EB]">
                  {rows.map((row, index) => (
                    <tr key={row.course_id} className="hover:bg-[#F9FAFB]">
                      <td className="px-4 py-4 font-semibold text-[#6B7280]">{(page - 1) * limit + index + 1}</td>
                      <td className="px-4 py-4 font-semibold text-[#111827]">{row.course_name}</td>
                      <td className="px-4 py-4 font-mono text-xs font-semibold text-[#800000]">{row.course_code}</td>
                      <td className="px-4 py-4 font-semibold text-[#111827]">{row.department?.faculty?.faculty_name ?? "—"}</td>
                      ...
                      <td className="px-4 py-4 font-semibold text-[#111827]">{row.semester?.academic?.year ?? "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
